function [Ycap]=call_bcdLrMr(X,opts)
...
[L_vec,M_vec,lsearch,comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit]=scanparam(defopts,opts);

if isempty(L_vec)||isempty(M_vec)
    fprintf(2,'L_vec, M_vec must be specified.');
    Ycap=[];
    return;
end
...
X=double(X);
[A,B,C,phi,it1,it2,phi_vec]=bcdLrMr_alsls(X,L_vec,M_vec,lsearch,comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit);
Ycap.A=A;
Ycap.B=B;
Ycap.C=C;
Ycap.phi=phi;
