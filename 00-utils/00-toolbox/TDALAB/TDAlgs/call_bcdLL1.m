function [Ycap]=call_bcdLL1(X,opts)
%BCDLL1_ALSLS Block Component Decomposition (BCD) in rank-(L,L,1) terms.
%   [A,B,C]=bcdLL1_alsls(X,R,L) computes the BCD-LL1 of a third-order tensor.
...
% of size (I,J,K) in a sum of R terms of rank-(L,L,1): 
% X = (A1 x2 B1 x3 c1)  + (A2 x2 B2 x3 c2) + ... + (AR x2 BR x3 cR) where xn is 
% the mode-n tensor-matrix product
% A = [A1 A2 ... AR] is of size (I,RL), i.e. each block Ar is of size (IxL),
% B = [B1 B2 ... BR] is of size (I,RL), i.e. each block Br is of size (JxL),
% C = [c1 c2 ... cR] is of size (K,R)
%
% INPUTS: 
% - X: tensor of size (IxJxK)  
% - R: number of rank-(L,L,1) terms
% - L: number of columns of the matrices Ar and Br, r=1...R
%
% OPTIONAL INPUTS:
...
%
% OUTPUTS:
% - A (IxRL), B(JxRL), C(KxR): estimates of the loading matrices
...
% (a)- If the dimensions allow it, namely if I>=RL and J>=RL, the first 
...
[R,L,lsearch,comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit]=scanparam(defopts,opts);

if isempty(R)||isempty(L)
    fprintf(2,'R,L must be specified.');
    Ycap=[];
    return;
end

X=double(X);
[A,B,C,phi,it1,it2,phi_vec]=bcdLL1_alsls(X,R,L,lsearch,comp,Tol1,MaxIt1,Tol2,MaxIt2,Ninit);
Ycap.A=A;
Ycap.B=B;
Ycap.C=C;
Ycap.phi=phi;
