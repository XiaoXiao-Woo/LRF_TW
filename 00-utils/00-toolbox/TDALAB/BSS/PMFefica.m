function [S A]=PMFefica(X,opts)
%% EFICA: [S A]=PMFefica(X,opts)
%  opts.ini: initialization, can be 'randn' or 'eye'.
%      .NumOfComp: number of sources/components.
%      .SaddleTest: if true (default) the test of saddle points on the demixed signals is done
%  X=S*A
%     where the columns of S are the source signals.
%
% Based on the version: 2.12  release: 11.6.2009
%
...
defopts=struct('NumOfComp',[],'initmode','randn','SaddleTest',true);
if ~exist('opts','var')
    opts=struct;
end
[m,ini,SaddleTest]=scanparam(defopts,opts);
if isempty(m)
    m=size(X,2);
end

[z temp1 temp2]=svds(X,m);
z=z';
...
S=efica(z, ini, SaddleTest);
S=S';
A=S\X;

function [icasig]=efica(X, ini, SaddleTest)
