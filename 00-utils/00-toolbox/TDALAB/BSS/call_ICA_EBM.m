function [S A]=call_ICA_EBM(X,opts)
% Call the ICA-EBM algorithm: ICA by Entropy Bound Minimization (real-valued version).
% The original version: function W = ICA_EBM(X);
% Revised for tdalab.
% Parameters:
% opts.
%     NumOfComp: Number of components
%     lra: none|pca|sampling|randompca dimensionality reduction algorithm
%     lra_p: parameter for low-rank approximation (dimensionality
%     reduction)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
defopts=struct('NumOfComp',[],'lra','none','lra_p',[],'max_iter_fastica',100,'max_iter_orth',1000,...
...
[NumOfComp lra lra_p max_iter_fastica,max_iter_orth,max_iter_orth_refine,max_iter_nonorth,...
    max_cost_increase_number,stochastic_search_factor,saddle_test_enable,...
    verbose]=scanparam(defopts,opts);
Y=double(X);
if isempty(NumOfComp)
    NumOfComp=size(Y,2);
end
if NumOfComp<size(Y,2)&&strcmpi(lra,'none')
    lra='pca';
end
if ~strcmpi(lra,'none')
    X=lowrankapp(Y,NumOfComp,lra,lra_p)';
else
    X=Y';
end

%% run ICA
W=ICA_EBM(X,max_iter_fastica,max_iter_orth,max_iter_orth_refine,max_iter_nonorth,...
            saddle_test_enable,max_cost_increase_number,stochastic_search_factor,verbose);
S=(W*X)';
A=S\Y;
        

function W = ICA_EBM(X,max_iter_fastica,max_iter_orth,max_iter_orth_refine,max_iter_nonorth,...
            saddle_test_enable,max_cost_increase_number,stochastic_search_factor,verbose)
% max_iter_fastica = 100;
% max_iter_orth = 1000;
% max_iter_orth_refine = 1000;
