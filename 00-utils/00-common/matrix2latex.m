...
% Enjoy life!!!
    ...
    if isnumeric(matrix)
        ...
        for h=1:height
            for w=1:width
                label = colLabels(w);
                if isfield(format, label)
%                 if(~isempty(format(w)))
                    matrix{h, w} = num2str(matrix{h, w}, getfield(format, label));
                else
                    matrix{h, w} = num2str(matrix{h, w}, "%.4f");
                end
            end
        end
    end
    ...
    for h=1:height
        ...
        for w=1:width-1
            if mod(w, 2) ~= 0 && flag_std
                fprintf(fid, '%s $\\pm$ ', matrix{h, w});
            else
